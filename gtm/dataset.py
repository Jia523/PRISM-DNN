...
class GeoPretrainDataset(Dataset):

    def __init__(self, csv_data: pd.DataFrame, input_cont_cols: List[str], input_cate_cols: List[str],
                 space_target_cols: List[str], time_target_cols: List[str]):
        self.csv_data = csv_data
        self.num_cont_feats = len(input_cont_cols)
        self.num_space_outs = len(space_target_cols)
        if time_target_cols:
            self.num_time_outs = len(time_target_cols)
            self.time_targets_data = self.csv_data[time_target_cols].values.astype(np.float32)
        else:
            self.num_time_outs = 0
            self.time_targets_data = None
        ...
        self.space_targets_data = self.csv_data[space_target_cols].values.astype(np.float32)
    ...
    def __getitem__(self, idx):
        return {
            ...
            "SPACE_TARGET": self.space_targets_data[idx],
            "TIME_TARGET": self.time_targets_data[idx] if self.time_targets_data is not None else 0
        }


class GeoTaskDataset(Dataset):

    def __init__(self, csv_data: pd.DataFrame, input_cont_cols: List[str], input_cate_cols: List[str],
                 task_target_cols: List[str]):
        self.csv_data = csv_data
        self.num_cont_feats = len(input_cont_cols)
